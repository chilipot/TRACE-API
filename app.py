...
def get_all_reports():
	...
	if not (params.get('direct') is None):
		objectID = str(params.get('direct', 0))
		print(objectID)
		return json_util.dumps({'result' : report.find({"_id" : ObjectId(objectID)})})
...
def get_single_report(objectID):
	...
	if just_score and just_score.lower() == 'true':
