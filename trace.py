...
def report_info_data(driver, termID, count=100000000):
...
def download_scores(driver, reportID, instructorID, termID):
    driver.get("https://www.applyweb.com/eval/new/showreport/excel?r=2&c=" + str(reportID) + "&i=" + str(instructorID) + "&t=" + str(termID) + "&d=false")

def download_all_scores(driver, reports_noScores):
    for report in reports_noScores:
        download_scores(driver, report.id, report.instructor.instructorID, report.term.termID)
...
def get_all_reports():
    ...
    #with open('reports_noScores.json') as f:
