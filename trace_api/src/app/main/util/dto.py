...
class UserDto:
    ...
    user = api.model('user', {
        ...
        'admin': fields.Boolean(attribute='Admin', required=True, default=False,
                                description='user is admin?'),
        'password': fields.String(attribute='PasswordHash', required=True,
                                  description='user password'),
        'public_id': fields.String(attribute='PublicID',
...
class ReportDto:
    ...
    term = api.model('term', {
        'TermID': fields.Integer(attribute='TermID', required=True,
                                 description='term id'),
        'Title': fields.String(attribute='Title', required=True,
                               description='term name/title')
    })

    instructor = api.model('instructor', {
        'InstructorID': fields.Integer(attribute='InstructorID', required=True,
                                       description='instructor id'),
        'FirstName': fields.String(attribute='FirstName', required=True,
                                   description='instructor first name'),
        'MiddleName': fields.String(attribute='MiddleName', required=True,
    ...
    course = api.model('course', {
        'ReportID': fields.Integer(attribute='ReportID', required=True,
                                   description='report id'),
        'CourseID': fields.Integer(attribute='CourseID', required=True,
                                   description='course id'),
        'Instructor': fields.Nested(instructor),
        'Term': fields.Nested(term),
        'Name': fields.String(attribute='Name', required=True,
                              description='course name'),
        'Subject': fields.String(attribute='Subject', required=True,
                                 description='department code/subject of course'),
        'Number': fields.String(attribute='Number', required=True,
                                description='course number'),
        'Section': fields.String(attribute='Section',
                                 description='section code/number for course')
    })
    ...
    answer = api.model('answer', {
        ...
        'Number': fields.Integer(attribute='Value', required=True,
                                 description='number who chose this score')
    })

    question = api.model('question', {
        'Answers': fields.List(fields.Nested(answer)),
        'Text': fields.String(attribute='LookupQuestionText.Text',
                              required=True,
                              description='question being asked'),
        'ResponseCount': fields.Integer(
            description='number who responded to this question'),
    ...
    course_meta = api.model('course meta', {
        'Instructor': fields.String(attribute='Instructor.full_name',
                                    description='full name of instructor'),
        'Course': fields.String(attribute='course_full_name',
                                description='course full name'),
        'Term': fields.String(attribute='Term.normal_title',
                              description='cleaned term title')
    })
    ...
    report = api.model('report', {
        ...
        'Questions': fields.List(fields.Nested(question),
                                 attribute='ScoreDatum.Questions'),
        'Enrollment': fields.Integer(attribute='ScoreDatum.Enrollment',
                                     description='number of students in this course/section'),
        'Responses': fields.Integer(attribute='ScoreDatum.Responses',
                                    description='number of responses to the survey'),
